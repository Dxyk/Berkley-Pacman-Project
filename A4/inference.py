        ...
        * When a ghost is captured by Pacman, all beliefs should be
        ...
        for pos in self.legalPositions:
            ...
            if emissionModel[trueDistance] > 0:
                allPossible[pos] = self.beliefs[pos] * \
                                   emissionModel[util.manhattanDistance(pos,
                                                                        pacmanPosition)]
        # special case
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for pos in self.legalPositions:
            newPosDist = self.getPositionDistribution(
                self.setGhostPosition(gameState, pos))
            for newPos, prob in newPosDist.items():
                all_possible[newPos] += self.beliefs[pos] * prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = []
        particles_left = self.numParticles
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for particle in self.particles:
                ...
                weighted_particles.append((particle, emissionModel[distance]))

            weight_list = [particle[1] for particle in weighted_particles]

            if not any(weight_list):
                self.initializeUniformly(gameState)
            else:
                distribution = util.Counter()
                total_weight = sum(weight_list)
                particle_weight_dic = {}
                for particle in weighted_particles:
                    if particle[0] in particle_weight_dic.keys():
                        particle_weight_dic[particle[0]] += particle[1]
                    else:
                        particle_weight_dic[particle[0]] = particle[1]
                for key in particle_weight_dic.keys():
                    distribution[key] = particle_weight_dic[key] / total_weight
                self.particles = [util.sample(distribution) for i in
                                  range(0, self.numParticles)]
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            distribution = self.getPositionDistribution(
                self.setGhostPosition(gameState, p))
            particles_temp.append(util.sample(distribution))
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        if len(noisyDistances) < self.numGhosts:
            return
        emission_model_list = [busters.getObservationDistribution(dist) for dist
                               in noisyDistances]

        "*** YOUR CODE HERE ***"
        weighted_particles_list = []
        for particle in self.particles:
            weight = 1
            for i in range(self.numGhosts):
                if noisyDistances[i] is None:
                    ...
                    distance = util.manhattanDistance(pacmanPosition,
                                                      particle[i])
                    weight *= emission_model_list[i][distance]
            weighted_particles_list.append((particle, weight))

        weight_list = [particle[1] for particle in weighted_particles_list]
        ...
        if not any(weight_list):
            ...
            dist = util.Counter()
            particle_weight_dic = {}
            for pw in weighted_particles_list:
                if pw[0] in particle_weight_dic.keys():
                    particle_weight_dic[pw[0]] += pw[1]
                else:
                    particle_weight_dic[pw[0]] = pw[1]

            for particle in particle_weight_dic.keys():
                dist[particle] = particle_weight_dic[particle] / total_weight
            self.particles = [util.sample(dist) for i in
                              range(0, self.numParticles)]
        "*** END YOUR CODE HERE ***"

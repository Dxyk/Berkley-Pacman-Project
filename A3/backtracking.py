...
class UnassignedVars:
    ...
    def insert(self, var):
        if not var in self.csp.variables():
            print "Error, trying to insert variable {} in unassigned that is not in the CSP problem".format(
                var.name())
        else:
...
def FCCheck(cnstr, reasonVar, reasonVal):
    if cnstr.numUnassigned() != 1:
        print "Error FCCheck called on constraint {} with {} neq 1 unassigned vars".format(
...
def FC(unAssignedVars, csp, allSolutions, trace):
    ...
    if unAssignedVars.empty():
        soln = [(var, var.getValue()) for var in csp.variables()]
        return [soln]
    solns = []
    ...
    for val in next_var.curDomain():
        ...
        for constraint in csp.constraintsOf(next_var):
            if constraint.numUnassigned() == 1:
                if FCCheck(constraint, next_var, val) == "DWO":
                    noDWO = False
                    break
        if noDWO:
            ...
            if new_solns:
                solns.extend(new_solns)
            if len(solns) > 0 and not allSolutions:
                Variable.restoreValues(next_var, val)
                break
        Variable.restoreValues(next_var, val)
    next_var.unAssign()

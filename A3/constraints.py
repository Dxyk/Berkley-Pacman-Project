...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        scope = [qi, qj]
        satAssignments = []
        for row in qi.domain():
            for col in qj.domain():
                if not (abs(row - col) == abs(i - j)) and row != col:
                    satAssignments.append([row, col])
...
class NValuesConstraint(Constraint):
    ...
    def hasSupport(self, var, val):
        ...
        for variable in scope:
            if self._required in variable.curDomain() or variable.getValue() == self._required:
                possible_vars += 1
        if val == self._required:
            possible_vars += 1
        return self._ub >= possible_vars >= self._lb
